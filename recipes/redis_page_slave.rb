...
master_name, master_ip, master_port = MagentostackUtil.redis_find_masters(node) do |name, data|
  name.include?('-page-master') && !name.include?('slave')
end

unless master_name && master_ip && master_port
  Chef::Log.warn('Did not find a redis page master to configure a redis slave, not proceeding')
  return
end

bind_port = node['magentostack']['redis']['bind_port_page_slave']
server_name = "#{bind_port}-page-slave"
...
tag('magentostack_redis_page_slave')
